...
sexes.forEach((sex, i) => {
    ...
    const mapContainer = d3.select("#us-map");
    const mapWidth  = mapContainer.node().clientWidth  || 900;
    const mapHeight = mapContainer.node().clientHeight || 380;
    ...
    // =======================================
    // HELPER: STATE VALUE AGGREGATION
    // =======================================
    function computeStateValues(organ) {
        const base = getFilteredBase(organ);
        return d3.rollup(
            base,
            v => d3.mean(v, d => d.val),
    ...
    // =======================================
    // HELPER: DETAILED STATE DATA (RESPECTS FILTERS)
    // =======================================
    function getStateDetailData(stateName) {
        return data.filter(d =>
            d.location_name === stateName &&
            (!globalFilters.sex  || d.sex_name === globalFilters.sex) &&
            (!globalFilters.year || d.year     === globalFilters.year) &&
            (!globalFilters.age  || d.age_name === globalFilters.age)
        );
    ...
    // =======================================
    // UPDATE MAP FOR ORGAN
    // =======================================
    function updateMapForOrgan(organ) {

        const stateAgg = computeStateValues(organ);

        // collect numeric values for quantile scale
        const vals = Array.from(stateAgg.values()).filter(v => v != null);

        // edge case: if no states have data, clear map
        if (!vals.length) {
            statePaths.transition().duration(300).attr("fill", "#f5f5f5");
            return;
        }

        // build quantile scale
        const colorState = d3.scaleQuantile()
            .domain(vals)
            .range([
                "#2ECC71",  // green  (low)
                "#A5D834",  // yellow-green
        ...
        statePaths
            ...
            .attr("fill", d => {
                ...
                const val = stateAgg.get(stateName);
                return val != null ? colorState(val) : "#f5f5f5";
            });
    }


    // =======================================
    // RESET MAP (ALL ORGANS)
    // =======================================
    function resetMapHighlight() {
        updateMapForOrgan(null);
    }
    ...
    statePaths
        .on("mouseover", (event, d) => {
            const stateName = d.properties.name;
            const stateData = getStateDetailData(stateName);

            if (!stateData.length) {
                tooltip.style("opacity", 1)
                    .html(`<b>${stateName}</b><br>No data available.`)
                    .style("left", (event.pageX + 10) + "px")
                    .style("top", (event.pageY - 28) + "px");
                return;
            }

            const avgVal = d3.mean(stateData, x => x.val);

            const byOrgan = d3.rollup(
                stateData,
                v => d3.mean(v, x => x.val),
                x => x.Organ
            );

            // find organ with highest mean val
            let topOrgan = null;
            let topVal = -Infinity;
            for (const [org, v] of byOrgan.entries()) {
                if (v != null && v > topVal) {
                    topVal = v;
                    topOrgan = org;
                }
            ...
            tooltip.style("opacity", 1)
                .html(`
                    <b>${stateName}</b><br>
                    Avg mortality (all causes): <b>${(avgVal * 100).toFixed(2)}%</b><br>
                    Highest Risk Organ: <b>${topOrgan || "N/A"}</b>
                    ${topVal && isFinite(topVal) ? ` (${(topVal * 100).toFixed(2)}%)` : ""}
                `)
                .style("left", (event.pageX + 10) + "px")
                .style("top", (event.pageY - 28) + "px");
        })
